...
int connection_http::n = 0;

sockets_http::sockets_http(int port, logs *log) : tcp_server_abstract(port, log) {
    httpFile::initMime();
    ...
    while (exit == false) {
        ...
        conn = new connection_http(socket, log, this);
...
void connection_http::replaceEntities(string &str) {
...
reqParams connection_http::parseParams(const string & params) {
    reqParams results;
...
void connection_http::parseRequest(const clientReqData &req) {
...
clientReqData connection_http::getRequest() {
    clientReqData ret;
...
void connection_http::sendResponse(const char *content, const char *content_type, string status) {
    char * format = "\
HTTP/1.1 %s\n\
...
void connection_http::sendResponseString(const char *response) {
...
void connection_http::sendChunkedResponse(const char *content, const char *content_type) {
...
void connection_http::processDownload(bool chunked,string file_name) {
...
bool connection_http::processRequest() {
...
void connection_http::dumpRequest(clientReqData &req) {
